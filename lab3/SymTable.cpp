...
FunctionEntry::FunctionEntry(PrimitiveTypeNode* retType, std::vector<TypeNode*> paraTypes)
    : returnType(retType), parameterTypes(paraTypes), proto(false) {}
...
template<class T>
bool SymTable<T>::contains(const std::string &name) {return table.find(name) != table.end();}

template<class T>
T& SymTable<T>::get(const std::string &name) {return table[name];}

template<class T>
void SymTable<T>::insert(const std::string& name, T ent) {table[name] = ent;}
